...
"""Merlin UV layout DXF exporter.

The export half of the Merlin package: exports the UV island outlines
of the active mesh object as an AutoCAD-compatible DXF (R12) file, with
mirroring/rotation fixing and Freestyle-edge auto scaling.  This is the
"Export UV Layout to DXF (AutoCAD)" add-on repackaged under the Merlin
operator namespace; the geometry pipeline lives in :mod:`uv_layout` and
:mod:`dxf_writer`.
"""
...
class MERLIN_OT_export_uv_dxf(bpy.types.Operator, ExportHelper):
    """Export the active mesh's UV layout as an AutoCAD-compatible DXF file"""

    bl_idname = "merlin.export_uv_layout_dxf"
    bl_label = "Merlin Export UV Layout"
