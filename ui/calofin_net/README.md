...
`lzt:*max-steps*`, generated by asking `lzt:chart` for every count the
DCL dialog accepts. The palette offers exactly those counts, because
past the ceiling LAZSTEP will not draw at all.

State line, Recall last and the wire are the Pool chart's, through the
same `FormWire` kit. The recall slot carries the **count** as well as
the routine (`CORNERSTP-3`), which is `lzt:recall-slot`'s own rule: a
three-step sheet recalled onto a five-step drawing would put numbers
against treads they were never measured on.

**One rule is mirrored from `lzt:form` rather than left to the wire**,
and it is the only one in the palette: `NA` at a tread counts as an
empty box. `NA` is what *ends* a run, so a tread answered `NA` would
stop the flight short of the count that built the very drawing in front
