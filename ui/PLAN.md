...
| **The receiving end in Lisp** | **done, both tools.** `pool:*form*` / `pool:run-with-answers` and `spa:*form*` / `spa:run-with-answers`, hooks in the ask helpers, consume-once, cleared on both exits |
