    ...
    public static class CommandCatalog
    {
            ...
            new Dictionary<string, Item[]>
        {
            { "Layout", new[]
            {
                ...
                new Item(new Entry("LAZSIDE", "Side view from a filled-in section", "Read the section, type the letters beside it, and POOLSIDE draws the side view"), new Entry[0]),
                new Item(new Entry("LAZSPA", "Spa from a filled-in chart", "LAZFORM's argument applied to SPA - fill the chart in and the spa is drawn"), new Entry[0]),
                new Item(new Entry("SPA", "Spa template", "Spa / hot-tub template layout"), new Entry[0]),
                new Item(new Entry("SPACOVCREATE", "Spa cover from the spa", "Offsets a selected spa outline into its cover and hinges it to the taper"), new Entry[0]),
                ...
                new Item(new Entry("POOLSIDE", "Pool side view", "POOL's longitudinal section on its own, from the floor run chain"), new Entry[0]),
                new Item(new Entry("OASIS", "Freeform pool", "Continuous-tangent pool drawn live from envelope and radii"), new Entry[0]),
                ...
                new Item(new Entry("ADAB", "Organic shape points", "Freeform perimeter through surveyed points"), new Entry[0]),
                new Item(new Entry("LHD", "Laser outline fit", "Laser-point outline fit, open or closed"), new Entry[0]),
                new Item(new Entry("ABLOBF", "Open best-fit run", "Fits an OPEN run of arcs and lines through points, between two ends you pick"), new Entry[0]),
                ...
                new Item(new Entry("UPADOVER", "Pads a run of wall", "Pads a stretch of wall between two points, or the whole of a line or polyline - no overlap, no gap"), new Entry[0]),
                new Item(new Entry("AUTOBEAD", "Bead offsets", "Offsets selected pool lines toward a clicked side"), new Entry[0]),
                new Item(new Entry("LAZSTEP", "Steps from a filled-in drawing", "Type the step count and the drawing follows it - fill it in and the steps are drawn"), new Entry[0]),
                new Item(new Entry("CORNERSTP", "Corner step", "Corner step layout"), new Entry[0]),
                new Item(new Entry("HEMISTEP", "Hemi step", "Hemi step layout"), new Entry[0]),
                new Item(new Entry("NORMIESTEP", "Normie step", "Normie step layout"), new Entry[0]),
                ...
                new Item(new Entry("STOCKCOVER", "Stock cover placement", "Replaces a highlighted perimeter with a stock cover drawing"), new Entry[0]),
                new Item(new Entry("WCALST", "Unroll curved band", "Unrolls a curved constant-width band flat, with darts"), new Entry[0]),
                new Item(new Entry("CUSTBLOCK", "Block from L/W/H", "Custom block in pictorial view from three typed sizes"), new Entry[0]),
                new Item(new Entry("SQUAREUP", "Square up to the perimeter", "Turns a highlighted drawing until its perimeter's longest wall (or span) is horizontal"), new Entry[0]),
            } },
            { "Points", new[]
            {
                ...
                new Item(new Entry("XYPLOT", "X/Y offset plot", "Plot an X/Y sheet, twice: points, and dimensioned"), new Entry[0]),
                new Item(new Entry("CONSTELLATION", "Points from cross dims", "Places points from the distances between them, inside a known box"), new Entry[0]),
                new Item(new Entry("LOBF", "Line of best fit", "Fits a construction line through points that should be on one line"), new Entry[0]),
                new Item(new Entry("ABFIND", "A/B stake ties", "Ties Pt.## back to the A and B survey stakes"), new Entry[0]),
                new Item(new Entry("ABMOVE", "Move mis-taped point", "Moves a point, offering every mis-read tape it could be"), new Entry[0]),
                new Item(new Entry("ABPCREATE", "Create a missing point", "Plots a point that is not there yet from the two readings it was taped at"), new Entry[0]),
                new Item(new Entry("POINTRENAMER", "Renumber points in order", "Hands the survey point numbers back out in perimeter order"), new Entry[0]),
                ...
                new Item(new Entry("PERPMARK", "Measured wall offsets", "Name a survey point, type what it measured: circle, perpendicular, dimension"), new Entry[0]),
                new Item(new Entry("DRONE", "Drone cleanup", "Drone cleanup routine"), new Entry[0]),
                new Item(new Entry("TYDRN", "Text + point tidy-up", "Text, pool-point and anchor cleanup in one pass"), new Entry[0]),
                new Item(new Entry("TYLERDRONESUITE", "Drone suite: tidy, pad, CDIM", "The whole drone trace in one - TYDRN, then PADDLE, then CDIM"), new Entry[0]),
            } },
            { "Dimensions", new[]
            {
                ...
                new Item(new Entry("STAIRDIM", "Stair dims", "Stair dimensioning"), new Entry[0]),
                new Item(new Entry("FLOORDIM", "Floor dims", "Floor dimensioning"), new Entry[0]),
                new Item(new Entry("DIMCONTEND", "Continue dim chains", "Chains a seed dimension out to every feature point"), new Entry[0]),
            ...
            { "Checking", new[]
            {
                new Item(new Entry("CHECK", "Drawing check", "General drawing check"), new Entry[0]),
                new Item(new Entry("DIMARCCHECK", "Arc endpoint check", "Dim arc endpoint check"), new Entry[0]),
                ...
                new Item(new Entry("OLAUTO", "Overlay two perimeters", "Best-fit overlay of a new perimeter on the original, worst error dimensioned"), new Entry[0]),
                new Item(new Entry("ABPCHECK", "Survey point offsets", "ABHD's measuring half as a checker - how far each point is off the line"), new Entry[0]),
                new Item(new Entry("LINCHECK", "Line checklist", "Line / text check"), new Entry[0]),
                ...
                new Item(new Entry("LINTXTCHK", "Liner checklist text", "Places the vinyl-liner QA checklist as drawing text"), new Entry[0]),
                new Item(new Entry("CCPRECHECK", "Tech flow chart", "Walks the Tech Flow Chart decision tree"), new Entry[0]),
                new Item(new Entry("LAZDIAG", "Error report for the last failure", "Write the last failure out as a DXF to send in - and, with nothing to report, prove that path works"), new Entry[0]),
                new Item(new Entry("LAZLOG", "What every command has done lately", "Every calofin command that finished, was backed out of or FAILED - the log a report's history comes from"), new Entry[0]),
            } },
        };
    }
}
