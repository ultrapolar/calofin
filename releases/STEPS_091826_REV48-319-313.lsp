;;; ======================================================================
;;; STEPS_091826_REV48-319-313.lsp
;;; ----------------------------------------------------------------------
...
;;;
;;;     CORNERSTP.lsp   v4.8 -> REV48   CORNERSTP, TUTORIALCORNERSTP, CORNERSTPVER
;;;     HEMISTEP.lsp    v3.19 -> REV319   HEMISTEP, TUTORIALHEMISTEP, HEMISTEPVER
;;;     NORMIESTEP.lsp  v3.13 -> REV313   NORMIESTEP, TUTORIALNORMIESTEP, NORMIESTEPVER
;;;
...
;;; ======================================================================
;;; >>> CORNERSTP.lsp (v4.8) - verbatim from lisp/cornerstp/CORNERSTP.lsp
;;; ======================================================================
;;; ======================================================================
;;; CORNERSTP.lsp
;;; ----------------------------------------------------------------------
...
;;; ======================================================================
;;; >>> HEMISTEP.lsp (v3.19) - verbatim from lisp/cornerstp/HEMISTEP.lsp
;;; ======================================================================
;;; ======================================================================
;;; HEMISTEP.lsp
;;; ----------------------------------------------------------------------
...
;; Returns (ENTS BOX ROWS): the entities drawn, BOX as (XMIN XMAX YTOL)
;; for the hit test, and ROWS as (EIGHTHS ROW-Y) pairs.
(defun hs-draw-ruler (total-eighths hasfeet lay style / rows n i row val
                          tier y hgt tl spx ents result view vx vy vw vh
                          gap base rcol dir far near)
  (setq rows (cons (list total-eighths 'current)
...
;; down when LEN is nil, since there is nothing to build one round.
;; The one-line hint is said the first time a run draws one.
(defun hs-ruler-show (state len / rr)
  (cond
    ((null len) (hs-ruler-off state))
    ((and (nth 0 state) (equal (nth 0 state) len)) state)
    (T
     (setq state (hs-ruler-off state))
     (setq rr (hs-draw-ruler (hs-len-eighths len) (nth 1 state)
                              (nth 5 state) (nth 6 state)))
     (if (not (nth 7 state))
       (princ (strcat "\n  A ruler of nearby lengths is beside the"
                      " drawing: click a row to take it, or type a"
...
;; answer, and the ruler as it now stands.
;;
;; The caller SHOWS the ruler first -- (setq rl (hs-ruler-show rl
;; last) rr (hs-ask-len prompt kws rl) v (car rr) rl (cadr rr)) --
;; and that order is not a nicety: an Esc inside this prompt runs the
;; caller's *error*, and what that handler can take down is the ruler
;; the CALLER's state names.  A ruler drawn in here, in a state only
;; this function held, would outlive the Esc.  The caller keeps the
...
;; before a prompt that does not take it and on every way out.
(defun hs-ask-len (prompt kws state / pk v out done)
  (setq done nil out nil)
  (while (not done)
    (if kws (initget 128 kws) (initget 128))
    (setq pk (getpoint prompt))
           ...
           (while (eq dep 'RETRY)
             (cond
               ...
               (T
                ;; Undo is the old keyword, kept as a hidden synonym; the
                ;; length ruler stands round the last tread
                (setq rl  (hs-ruler-show rl lastdep)
                      rr  (hs-ask-len
                            (strcat "\nStep " (itoa n)
                                    " - step tread [Back"
                                    (if lastdep "/Same" "") "]"
...
;;; ======================================================================
;;; >>> NORMIESTEP.lsp (v3.13) - verbatim from lisp/cornerstp/NORMIESTEP.lsp
;;; ======================================================================
;;; ======================================================================
;;; NORMIESTEP.lsp
;;; ----------------------------------------------------------------------
...
;; Returns (ENTS BOX ROWS): the entities drawn, BOX as (XMIN XMAX YTOL)
;; for the hit test, and ROWS as (EIGHTHS ROW-Y) pairs.
(defun ns-draw-ruler (total-eighths hasfeet lay style / rows n i row val
                          tier y hgt tl spx ents result view vx vy vw vh
                          gap base rcol dir far near)
  (setq rows (cons (list total-eighths 'current)
...
;; down when LEN is nil, since there is nothing to build one round.
;; The one-line hint is said the first time a run draws one.
(defun ns-ruler-show (state len / rr)
  (cond
    ((null len) (ns-ruler-off state))
    ((and (nth 0 state) (equal (nth 0 state) len)) state)
    (T
     (setq state (ns-ruler-off state))
     (setq rr (ns-draw-ruler (ns-len-eighths len) (nth 1 state)
                              (nth 5 state) (nth 6 state)))
     (if (not (nth 7 state))
       (princ (strcat "\n  A ruler of nearby lengths is beside the"
                      " drawing: click a row to take it, or type a"
...
;; answer, and the ruler as it now stands.
;;
;; The caller SHOWS the ruler first -- (setq rl (ns-ruler-show rl
;; last) rr (ns-ask-len prompt kws rl) v (car rr) rl (cadr rr)) --
;; and that order is not a nicety: an Esc inside this prompt runs the
;; caller's *error*, and what that handler can take down is the ruler
;; the CALLER's state names.  A ruler drawn in here, in a state only
;; this function held, would outlive the Esc.  The caller keeps the
...
;; before a prompt that does not take it and on every way out.
(defun ns-ask-len (prompt kws state / pk v out done)
  (setq done nil out nil)
  (while (not done)
    (if kws (initget 128 kws) (initget 128))
    (setq pk (getpoint prompt))
           ...
           (while (eq dep 'RETRY)
             (cond
               ...
               (T
                ;; Undo is the old keyword, kept as a hidden synonym; the
                ;; length ruler stands round the last tread
                (setq rl  (ns-ruler-show rl lastdep)
                      rr  (ns-ask-len
                            (strcat "\nStep " (itoa n)
                                    " - step tread [Back"
                                    (if lastdep "/Same" "") "]"
