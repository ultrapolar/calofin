...
echo "  - EVERY command reports its failures: an *error* handler that calls"
echo "    lzd:report, lzd:begin at the top, lzd:end before the closing"
echo "    (princ), lzd:watch after a selection and lzd:ask at EVERY input"
echo "    (wrapped where the answer is read in place).  A NEW tool gets these"
echo "    too - write the handler, then python3 tools/check_lazdiag.py --fix"
echo "    wires the rest.  tools/probe_report.py REPLAYS a failure report"
...
echo
echo "Skills:  .claude/skills/ holds the working subset of the 260KB of prose --"
echo "         calofin-lisp (edit a tool), calofin-new-tool (add one),"
echo "         calofin-checks (a red check).  Their scripts save the most:"
echo "           S=.claude/skills/calofin-lisp/scripts"
