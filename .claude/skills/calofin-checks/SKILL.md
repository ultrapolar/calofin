...
| SPACE | a prompt, hint or caption shows `44 1/2` or `4'-4 1/2"`, or a prompt shows `1524 MM`, where the spacebar is Enter, so it arrives as two answers | dash it (`44-1/2`, `4'-4-1/2"`) or close the unit up (`1524mm`). A `(getstring T ...)` prompt is exempt, unless the same file reads text at an `(initget 128)` prompt; so is a self-test table (`X:selftests`), whose strings are values a report compares, never shown |
