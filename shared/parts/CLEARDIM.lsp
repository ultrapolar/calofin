...
(defun cd:on-track (base u s) (cal:v+ base (cal:v* u s)))
...
(defun cd:dim-meas (ed / dtype p13 p14 ang v meas)
  (setq dtype (logand 7 (cd:num 70 ed 0))
        ...
        p14   (cd:dxf 14 ed))
  (cond
    ((and (= dtype 1) p13 p14) (rtos (distance (cal:2d p13) (cal:2d p14))))
    ((and (= dtype 0) p13 p14)
...
(defun cd:r-box (r s / centre pad)
  (setq centre (cal:v+ (cd:on-track (cd:r-base r) (cd:r-u r) s) (cd:r-off r))
  ...
  (if (and ed (= (cd:dxf 0 ed) "DIMENSION"))
    (progn
      ...
      (if (null p11)
        ...
        (setq p11 (cal:v+ base
                         (cal:v+ (cal:v* u (if (and p13 p14)
                                           (* 0.5 (+ (cal:dot (cal:v- p13 base) u)
                                                     (cal:dot (cal:v- p14 base) u)))
                                           0.0))
                                (cal:v* (cal:perp u) (* 0.5 h))))))
      (setq s0  (cal:dot (cal:v- p11 base) u)
            off (cal:v- (cal:v- p11 base) (cal:v* u s0)))
      ...
      (if (and p13 p14)
        (progn
          ...
          (setq own (list (list f13 f14)
                          (list (cal:2d p13) (cal:v+ f13 (cd:ext-tip p13 f13 exe)))
                          (list (cal:2d p14) (cal:v+ f14 (cd:ext-tip p14 f14 exe)))))))
...
(defun cd:home (r / own)
  (setq own (cd:r-own r))
  (if own
    (* 0.5 (+ (cal:dot (cal:v- (caar own) (cd:r-base r)) (cd:r-u r))
              (cal:dot (cal:v- (cadar own) (cd:r-base r)) (cd:r-u r))))
    (cd:r-s0 r)))
...
(defun cd:row (r rowtol)
  (fix (/ (cadr (cal:v+ (cd:on-track (cd:r-base r) (cd:r-u r) (cd:r-s0 r))
                       (cd:r-off r)))
          rowtol)))
...
(defun cd:order-lt (a b / ra rb xa xb)
  ...
  (cond
    ((/= ra rb) (> ra rb))
    (T
     (setq xa (car (cal:v+ (cd:on-track (cd:r-base a) (cd:r-u a) (cd:r-s0 a))
                          (cd:r-off a)))
           xb (car (cal:v+ (cd:on-track (cd:r-base b) (cd:r-u b) (cd:r-s0 b))
                          (cd:r-off b))))
     (cond ((/= xa xb) (< xa xb))
  ...
  (if (not (cd:res-moved res))
    ...
    (progn
      (setq ed    (entget (cd:r-en r))
            p     (cal:v+ (cd:on-track (cd:r-base r) (cd:r-u r) (cd:res-s res))
                         (cd:r-off r))
...
(defun cd:report (what results moving / nmove nstuck nclear skip r why d res)
  ...
  (princ (strcat "\n  "
                 (if moving
                   (cal:plural nmove "slid clear along its own dimension line"
                              "slid clear along their own dimension lines")
                   (cal:plural nmove "to slide clear along its own dimension line"
