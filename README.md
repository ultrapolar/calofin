...
| `gen_ui_data.py` | Writes the palette's `Generated/CommandCatalog.g.vb` from `lzp:*captions*` / `lzp:*groups*` / `lzp:*tutorials*` plus `ui/calofin_net/blurbs.txt` and `ui/calofin_net/howto.txt`. `--check` fails when the file on disk is not what a fresh run would write, the same contract `releases/` is held to |
