...
| ABHD + ADAB (AutoLISP, AutoCAD 2018+) | `pool_fit_lisp/` | Builds a smooth closed polyline of long arcs running point-to-point, joints within 8° of tangent, as few curves as possible, radii snapped to feet/half-feet/inches, optional curve cap — through pool-edge survey points, guided by a drawn perimeter, a connect-the-dots sketch, or the points alone. Offers three candidate fits in colour to pick from, rings the points it could not hold, and can then draw the pool bottom: both break lines, the hopper as a blended inward offset, straight or curve-guided slope lines, all auto-dimensioned. ADAB runs just the bottom flow over an existing perimeter, and TUTORIALABHD teaches new users by checklist or on-screen demo. Ships twice: `abhd.lsp` (static autoload name) plus a version-named twin (`ABHD_MMDDYY_REV##.lsp`) — see `pool_fit_lisp/README.md` |
