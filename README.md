...
| `LAZPANEL`, `LAZBUTTON`, `LAZPIN`, `LAZICON`, `LAZPANELVER` | `lisp/lazpanel/` | A clickable button panel with the 64 headline drafting commands above - the zero-install GUI: the dialog is plain DCL that the file writes for itself at run time, so there is no DLL to `NETLOAD` and no second file to ship. Loading it also puts a one-button toolbar ("LazPanel", an orange hexagon it generates itself) on screen that you can drag anywhere or dock - click it to open the panel, or `LAZBUTTON` to re-summon it. The button is drawn at 32 pixels rather than 16, because the small one is easy to miss; AutoCAD sizes every toolbar together, so `LAZBUTTON` says out loud that the rest grew with it and `(setq lzp:*bigbutton* nil)` then `LAZBUTTON` puts them all back (after loading, not before - the load sets it itself). `TYLERDRONESUITE` gets a one-button toolbar of its own when the drone lisp is loaded ON ITS OWN - an orange TRIANGLE against the panel's hexagon, since both are painted the one orange and shape is the only thing that can tell them apart. Inside LAZPASS it does NOT: the panel is already on screen and carries the suite like every other tool, so the whole build puts up one external button and that one is the panel's. The test is `cal:*build-loading*`, which the loader and the bundle both raise; `lzp:*suitebutton*` overrules it either way, set AFTER loading. An edition states both tunables outright rather than leaving one to `AUTO`: `AUTO` reads `cal:*build-loading*`, which LAZPASS raises and nothing lowers, so a machine that had loaded LAZPASS earlier made the drone edition put up no button at all. A button the current build does not want is taken off the strip rather than merely skipped - AutoCAD keeps toolbars in the CUI, so whichever build was loaded LAST is the one whose buttons are showing; the other is hidden, not deleted, and comes back where you docked it. Tabs come in two rows: the **jobs** (Pool / Cover / Spa / Rest), each laid out in columns that follow the work - shape, points, steps, dims and check - and the **categories** (Layout / Points / Dimensions / Checking, the same four group names as the VB palette) holding the whole roster filed by what each tool is. A tool that serves two jobs sits on both, so 64 commands make 142 buttons; `Rest` is computed as whatever the three named jobs leave over. A command not loaded in this session is greyed out. A click closes the panel, runs the command exactly as if typed, and the panel then REOPENS itself on the same page at the same position - Close is the way out. A **Pinned** row on every page carries the few tools you run all day, remembered between sessions in the registry; `Pin...` or `LAZPIN` edits it. Off the panel on purpose: the satellites (`TUTORIAL*`, `*VER`, `*RESCUE`, `-CFG`/`-SETUP`, `DCE`, `STOCKLIST`), the `DD*` drone-height toolset, `LISPLAB` and the deprecated matcher - see `lisp/lazpanel/README.md` |
