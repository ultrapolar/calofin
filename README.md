...
| `DIMSTAMP`, `DIMSTAMPVER` | `lisp/dimstamp/` | Click a point, type a feet/inch/fraction dimension text, and it lands there as `TEXT` on `DIMENSION`; click again and Enter repeats the SAME text, or pick one of a short numbered menu of nearby values, or type `New` for something else entirely - whichever you land on becomes what the next click offers. Every value is one of four forms: `34"`, `3'-4"`, `34 1/2"` or `3'- 4 1/2"`. A bare-inch value with no fraction yet suggests every eighth of an inch up to the next whole inch; a feet-and-inches value suggests quarter-inch steps up and down plus 1"/2"/3" jumps on each side - eighth-inch steps instead of quarters once the text already carries a fraction, feet or not |
