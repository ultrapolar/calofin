...
| `DIMSTAMP`, `DIMSTAMPVER` | `lisp/dimstamp/` | Click a point, type a feet/inch/fraction dimension text, and it lands there as `TEXT` on `DIMENSION`. Beside it a scratch RULER appears - a vertical column of nearby values, each a tick and a label graded like a real tape measure: eighth-inch steps are the smallest text and shortest ticks, quarters and halves step up from there, and the whole-inch jumps (1"/2"/3" either way) are tallest and boldest; a small circle rides the row that is the current value. From there one prompt does three jobs: click empty space to stamp the current text again (the ruler follows), click a row on the ruler to adopt that value without stamping, or type a new value outright - Enter ends the run. The ruler is scratch, not drawing content, and is swept away as it redraws and for good when the run ends; only the stamped `TEXT` survives. Every value is one of four forms: `34"`, `3'-4"`, `34 1/2"` or `3'- 4 1/2"`. A bare-inch value suggests every eighth up to the next whole inch; a feet-and-inches value suggests eighth-inch steps up and down (quarters and eighths together, told apart by tier) plus 1"/2"/3" jumps on each side |
...
`LAZPANEL` is pure AutoLISP, ships inside `LAZPASS.lsp`, covers the 87
