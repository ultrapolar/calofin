...
| --- | --- | --- |
| `POOL`, `POOLCOVER`, `POOLDEMO`, `POOLDEMOVER`, `POOLVER`, `TUTORIALPOOL` | `lisp/pool/` | As-built pool plan generator - Rectangle, Oval, Grecian, L, Lazy L - from field measurements. `POOLCOVER` is the same command for a cover sheet: the pool-bottom question is answered No before it is asked, so the depth chain behind it (C, C2, D, the hopper type and its corner method) never runs. Every corner SIZE and every DEPTH stands beside `DIMSTAMP`'s **ruler**, drawn as a LADDER: `Radius for <corner>` and `Cut face length for <corner>` offer 3" to 2'-0" in 3" steps, C offers 36"-54" by 3", D 60"-96" by 6" and C2 the span of the two, and clicking a rung IS the size -- a corner radius is picked out of the short list a shop builds to, not taped off the sheet. The WALL dims, the diagonals, the cross dims and the hopper runs beside them are measured, so they stay the plain typed questions they were |
