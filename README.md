...
| `PERPMARK`, `PERPMARKVER` | `lisp/perpmark/` | `PERPPTS` for the survey that arrives as a list of spots rather than an even spacing - a bench, a ledge, a gutter, anything taped square off the wall at the places it changes. Select the perimeter, click the centre of the pool once (which is the whole of the direction question - a mark runs toward the side the centre is on, worked out per mark off the wall's own tangent, so a run round a corner or along a radius each come off square), then click a spot and type what it measured, for as long as the sheet lasts. Each spot gets a circle of that radius - the swing of the tape - and a line of that length running square off the wall; a click that misses the wall is projected onto it, so a snap that went astray still marks the right place. `Back` takes the last mark away again, `Enter` ends the round. Then, if the marks are meant to BE something, it joins them: pick where the run starts and where it ends and one polyline goes through the far end of every mark between them, in the order the WALL runs rather than the order they were clicked, wrapping past a closed polyline's own seam when that is the way the two picks point. A run end that lands on a mark takes that mark's distance; one that lands anywhere else measures zero, which is how a step that dies back into the wall is drawn. The circles then go and every line becomes a `SIDE STANDARD` dimension - or, answered `No`, every circle and every line is left exactly where it is |
...
`LAZPANEL` is pure AutoLISP, ships inside `LAZPASS.lsp`, covers the 85
...
python3 tests/test_perpmark.py        # PERPMARK, the clicked-station
