...
canonical, actively-developed versions of those tools, and BOTH carry
the receiving end a form needs - an answer store the ask helpers read
before they prompt: `pool:*form*` / `spa:*form*`,
`pool:run-with-answers` / `spa:run-with-answers`, and the hooks
described under `LAZFORM` above. `tests/test_pool_form.py` and
`tests/test_spa_form.py` pass at both tiers because of it, and each
ends by checking the palette's field map sends no key its routine
cannot read.

`ui/PLAN.md` records how the gap was closed and what remains palette-side.
...
python3 tests/test_spa_form.py        # a form drives SPA the same way
