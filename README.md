...
| `PERPMARKSTAMP`, `PERPMARKSTAMPVER` | `lisp/perpmarkstamp/` | `PERPMARK` with the distance STAMPED beside every mark as it is drawn: every question is `PERPMARK`'s, and after each mark one more asks where to stamp the distance -- Enter puts it at the far end of the mark, a click puts it there, `Skip` leaves it off, `Back` takes the mark away. The stamp is `DIMSTAMP`'s MTEXT (layer `TEXT`, style `Attributes`, 6" high, the fraction stacked), spelled the way the distance was typed (`44 1/2"`, or `3'-8 1/2"` for one typed in feet), and it rides on its mark: `Back` and a re-mark take it away with the circle and the line, and the join leaves it standing beside the dimension. It copies nothing of `PERPMARK` -- it installs a hook `PERPMARK` v1.13 carries for it -- so it needs `PERPMARK` loaded, and says so when it is not |
...
`LAZPANEL` is pure AutoLISP, ships inside `LAZPASS.lsp`, covers the 97
...
python3 tests/test_perpmarkstamp.py   # PERPMARKSTAMP, PERPMARK with a
                                      #   stamp after every mark
