...
_state = {'*chk:log*', '*chk:prev*'}
_outside = [l for i, l in enumerate(_lines)
            if l.startswith('(setq chk:*') and not _start < i < _end
...
_undoc = [l for i, l in enumerate(_lines[_start:_end], _start)
          if l.startswith('(setq chk:*') and not _explained(i)]
assert not _undoc, _undoc
KNOBS = [k for k in _re.findall(r'^\(setq (chk:\*[a-z-]+\*)', _SRC, _re.M)
         if k not in _state]
_vm = VM()
_vm.load(LSP)
