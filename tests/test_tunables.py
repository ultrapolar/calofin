...
FILES = [
    ...
    # rest of what a shop tunes had drifted down beside the code that
    # read it: SPA's foam sheet and hardware rules -- the shop data its
    # whole hinge pass is built on -- sat 1,200 lines down, and POOL
    # spelled its three layer names out 67 times between them.
    ('POOL', ROOT / 'lisp' / 'pool' / 'POOL.LSP', 'pool',
     ROOT / 'lisp' / 'pool' / 'README.md', TUNABLES),
    ('SPA', ROOT / 'lisp' / 'spa' / 'SPA.LSP', 'spa',
     ROOT / 'lisp' / 'spa' / 'README.md', TUNABLES),
]
...
def assigned(src, ns):
    ...
    A STRING value takes a slot too, and missing that was a real bug:
    it made every pair AFTER a string in the same setq read
    name-for-value.  ``spa:setmode`` is one long setq with
    ``"WATER'S EDGE"`` in the middle of it, so the three knobs past
    that string -- read there, never written -- came back as writes,
    and check 3 called three settings state.  Strings now fall through
    to the slot count like any other atom.
    """
    out = {}
