...
def test_the_shades_can_be_switched_off():
    ...
    sf:colgroups already does with a nil shade."""
    vm = newvm()
    vm.loads('(setq sf:*shade-lo* nil)')
    e1 = line(vm, (0, 0), (100, 0))
    e2 = line(vm, (0, 0), (0, 100))
    run(vm, [[e1, [50.0, 0.0, 0.0]], [e2, [0.0, 50.0, 0.0]],
             ...
             'No'], 'no shades')
    assert 'corner cut' in said(vm), said(vm)
    assert vm.loads('(sf:shade 0 4)') is None, "no shade means no shade"
...
def test_every_exit_pops_the_error_mode():
    ...
    def drive(script, undoctl, label):
        ...
        picks = [[e1, [50.0, 0.0, 0.0]], [e2, [0.0, 50.0, 0.0]],
                 clicker(24.0)]
        vm.run('c:SMARTFILLET', script(picks) + [])
    ...
    for undoctl, uname in ((5, 'undo on'), (4, 'undo off')):
        ...
        e1 = line(vm, (0, 0), (100, 0))
        e2 = line(vm, (0, 0), (0, 100))
        vm.loads('(defun sf:dofillet (a b c d r) (sf:no-such-helper r))')
        vm.run('c:SMARTFILLET', [[e1, [50.0, 0.0, 0.0]], [e2, [0.0, 50.0, 0.0]],
                              clicker(24.0)])
        ...
        assert not alive(vm, layer=PREVIEW_LAYER)
        assert 'SMARTFILLET error' in said(vm), said(vm)
