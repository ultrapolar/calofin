...
| `*xft-record*` | `t` | write the undo record `XFTRECONV` reads; `nil` converts exactly as before and writes nothing down, so the run can only be undone by `U` |
| `*xft-xdata-app*` | `"XFTCONV"` | the xdata application the record lives under |
| `*xft-num-prec*` | `8` | decimals a coordinate is written to -- 8 puts the round trip within 1e-8 of a drawing unit |
| `*xft-rebuild-color*` | `7` | colour for a source layer `XFTRECONV` has to re-create because it was PURGED after the conversion; one still in the drawing keeps its own |
...
Last come the record and the revert: the trace fixture is converted and
`XFTRECONV` run over the result, which is where the 1e-8 figure above is
measured -- that fixture's coordinates carry more decimals than the record
writes down.
