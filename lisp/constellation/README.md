...
| `cst:*space-color*` | `8` | ACI colour the space layer is **created** with — grey, and a number rather than `'auto`, because the rectangle stays in the drawing and a drawing is opened by more than one drafter (LAZTUNE changes it per drafter); a frame rather than a result. A layer the drawing already has keeps its own colour and is only switched on, thawed and unlocked if it needs to be |
