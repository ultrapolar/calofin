...
(defun c:PERPMARK (/ *error* undo-open
  ...
  (while (not done)
    (cond
      ...
      ((= stage 31)
       (setq d (pm:askdist 'REQ "Distance from the perimeter at that point"
                           nil T))
       (cond
      ...
      ((= stage 5)
       ...
       (cond
         ((eq pick 'PM-BACK) (setq stage 4))
         ((null pick) (princ "\nA point is required."))
         (t
          (setq m0 (pm:runend en segs marks (pm:2d (trans pick 1 0))))
          (if (null m0)
            (princ (strcat "\nThe perimeter cannot be read there - pick"
                           " a point nearer the wall."))
            (setq stage 6)))))
      ...
      ((= stage 6)
       ...
       (cond
         ((eq pick 'PM-BACK) (setq stage 5))
         ((null pick) (princ "\nA point is required."))
         (t
          (setq m1 (pm:runend en segs marks (pm:2d (trans pick 1 0))))
          (if (null m1)
            (princ (strcat "\nThe perimeter cannot be read there - pick"
                           " a point nearer the wall."))
            (progn
