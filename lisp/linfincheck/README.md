...
`*lfc-grey-color*` is `'auto` because it has to work against either
...
carries, shared with COVERCHECK and DIMCHECK), so `'auto` resolves to
the number named below unless CALSET's Itemcolors menu has set a
`CalofinInk-<ROLE>` override in the profile. A number is still used
exactly as given, same as `*lfc-grey-color*`.
...
* **Demo** — draws a small practice drawing in an empty spot you pick,
  ...
  the practice drawing afterwards (`Erase the practice drawing? [Yes/No]
  <No>` -- Enter keeps it). The practice drawing goes on its own
