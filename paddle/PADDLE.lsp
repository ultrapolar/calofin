...
;;; PADDLE.lsp
;;;
;;; Scans the perimeter of a drawing (a closed polyline) for concave
;;; features that require pads, and inserts 24" x 24" pad blocks
;;; ("Pad24x24") to cover the affected areas.
;;;
;;; Pad specification:
...
;;;     corner) requires a pad nestled into the corner.
;;;   * Convex features and concave arcs larger than 4'-6" radius do
;;;     NOT require pads.
;;;
;;; Usage:
;;;   Command: PADDLE
;;;   Select the perimeter polyline(s), or press Enter to auto-detect
;;;   the perimeter (the largest closed polyline in the drawing).
;;;
...
(defun paddle--perp (v s) ; inward normal: +90 deg for CCW (s=1), -90 for CW
  (if (> s 0) (list (- (cadr v)) (car v)) (list (cadr v) (- (car v)))))

;; ----------------------- polyline extraction -----------------------
;; Returns a list of (x y bulge) for each vertex of an LWPOLYLINE,
;; consecutive duplicate points removed.
(defun paddle--verts (ent / ed out itm last)
  (setq ed (entget ent))
  (foreach grp ed
    (cond
      ((= (car grp) 10)
  ...
  ;; drop consecutive duplicates (incl. last == first)
  (setq last (car (reverse out)))
  (if (and (> (length out) 1)
           (< (distance (list (car last) (cadr last)) (list (caar out) (cadr (car out)))) 1e-8))
      (setq out (reverse (cdr (reverse out)))))
  (vl-remove nil
    (mapcar '(lambda (a b)
               (if (> (distance (list (car a) (cadr a)) (list (car b) (cadr b))) 1e-8) a))
            out (append (cdr out) (list (car out))))))
...
;; Signed area of the closed vertex list (shoelace + circular segments).
...
(defun paddle--features (vts / s n i a b c blg pads din dout turn nin nout ctr
                             seg theta r cen sa npads k f pang p u tang)
  ...
  (repeat n
    ...
    (setq din  (paddle--tan-end (list (car c) (cadr c)) (list (car a) (cadr a)) (caddr c))
          dout (paddle--tan-start (list (car a) (cadr a)) (list (car b) (cadr b)) blg))
    (if (and din dout)
        (progn
          (setq turn (atan (paddle--cross din dout) (paddle--dot din dout)))
          (if (< (* s turn) (- *paddle-angtol*)) ; turns away from interior
              (progn
                (setq nin  (paddle--perp din s)  ; inward edge normals
                      nout (paddle--perp dout s)
                      ctr  (paddle--add (list (car a) (cadr a))
                             (paddle--scl (paddle--add nin nout) (/ *paddle-padsize* 2.0))))
                (setq pads (cons (list ctr (angle '(0.0 0.0) din) "corner") pads))))))

    ;; --- concave arc segment with radius <= 4'-6" ---
    (if (and (/= blg 0.0)
             (< (* s blg) 0.0)) ; bulges into the interior
        (progn
          (setq seg   (paddle--arcdata (list (car a) (cadr a)) (list (car b) (cadr b)) blg)
                theta (car seg)
                r     (cadr seg)
                cen   (caddr seg))
          (if (<= r (+ *paddle-maxrad* 1e-6))
              (progn
                (setq sa    (angle cen (list (car a) (cadr a)))
                      npads (max 1 (fix (+ 0.999999 (/ (* r (abs theta)) *paddle-padsize*))))
                      k     0)
                (repeat npads
                  (setq f    (/ (+ k 0.5) npads)
                        pang (+ sa (* theta f))
                        p    (paddle--add cen (paddle--scl (paddle--dir pang) r))
                        u    (paddle--dir pang) ; away from center = into material
                        ctr  (paddle--add p (paddle--scl u (/ *paddle-padsize* 2.0)))
...
;; --------------------------- selection -----------------------------
;; Selected closed LWPOLYLINEs, or the largest closed one when SS is nil.
(defun paddle--perimeters (ss / i out ent best bestarea a)
  (if ss
      (progn
        (setq i 0)
        (repeat (sslength ss)
          ...
          (if (= 1 (logand 1 (cdr (assoc 70 (entget ent)))))
              ...
              (princ "\nPADDLE: skipping an open polyline (perimeter must be closed).")))
        (reverse out))
      ;; auto-detect: largest closed lwpolyline in the current tab
      (progn
        (setq ss (ssget "_X" (list '(0 . "LWPOLYLINE") '(-4 . "&=") '(70 . 1)
        ...
        (if ss
            (progn
              ...
              (repeat (sslength ss)
                (setq ent (ssname ss i) i (1+ i)
                      a   (abs (paddle--area (paddle--verts ent))))
                (if (> a bestarea) (setq bestarea a best ent)))))
        (if best
            (progn (princ "\nPADDLE: auto-detected the largest closed polyline as the perimeter.")
                   (list best))))))

;; ---------------------------- command ------------------------------
(defun c:PADDLE (/ *error* doc space ss perims ent vts pads allpads delta
                   ncorner narc)
  (defun *error* (msg)
  ...
  (princ "\nSelect perimeter polyline(s) or press Enter to auto-detect: ")
  (setq ss (ssget '((0 . "LWPOLYLINE"))))
  (setq perims (paddle--perimeters ss))

  (if (not perims)
      (princ "\nPADDLE: no closed perimeter polyline found.")
      (progn
