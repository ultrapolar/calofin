...
| grouped | `shared/parts/` | the same tools rebuilt against one helper library (`cal:` namespace), all loading together; `shared/LAZPASS.lsp` is the generated one-file build | generated by `tools/mirror_shared.py` — every twin but `LISPLAB.lsp`; bundle generated |
...
3. Mirror the change into `shared/parts/<FILE>.lsp`. Every twin but
   `LISPLAB.lsp` is **generated** — run
   `python3 tools/mirror_shared.py <TOOL>` and do not hand-edit it.
   LISPLAB's is by hand, and only because its twin's non-swap content
   is lesson prose naming the library instead of itself; a drift there
   fails `make parity`, which runs `test_lisplab.py` at both tiers.
4. Regenerate:
   ```
